    ...
    try {
      ...
      const emailFactor = factors.find(
        (factor) =>
          ...
          factor !== null &&
          "strategy" in factor &&
          (factor as { strategy: string }).strategy === "email_code"
      ...
      if (
        clerkError.errors?.[0]?.code === "form_identifier_not_found" ||
        clerkError.errors?.[0]?.message?.toLowerCase().includes("not found") ||
        clerkError.errors?.[0]?.message?.toLowerCase().includes("doesn't exist")
      ) {
        try {
          if (!signUp) {
          ...
          const clerkSignUpError = signUpErr as {
            errors?: Array<{ message?: string }>;
          };
          setError(
            clerkSignUpError.errors?.[0]?.message ||
              "Failed to send verification code"
          );
        }
      } else {
        setError(
          clerkError.errors?.[0]?.message || "Failed to send verification code"
        );
      }
    } finally {
      setIsLoading(false);
